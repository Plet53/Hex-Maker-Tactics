...
public class EditorControl : MonoBehaviour
{
  ...
  public prj tprj;
  public string filename, texname;
  ...
  IEnumerator Start()
  {
    //intialization pile
    maxHeight = 0;
    dispSelRect.SetActive(false);
    radDisp.SetActive(false);
    selRect.SetActive(false);
    paintMenu.SetActive(false);
    paintCreate.SetActive(false);
  ...
  {
    //initial click
    if(Input.GetMouseButtonDown(0)){initmp = Input.mousePosition;
      if(toolset[1])
      ...
      if(toolset[2]){radDisp.SetActive(true);
      ...
      if(toolset[0]){if(toolset[5]){Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
        if(Physics.Raycast(ray, out hit)){
          GameObject h = hit.collider.gameObject;
          create(h); d = true;}}
        if(toolset[7]){Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
        if(Physics.Raycast(ray, out hit)){
          GameObject h = hit.collider.gameObject;
          delete(h); d = true;}}}}
    //during clickhold
    if(Input.GetMouseButton(0)){if(toolset[6]){mp = Input.mousePosition;
      if((Mathf.Abs(mp.y - initmp.y)) > (canvtogrid * 40))
        ...
        initmp.y = mp.y;}}
      else{if(toolset[0]){Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
      if(Physics.Raycast(ray, out hit))
        {GameObject h = hit.collider.gameObject;
        if(toolset[3]){if((Input.GetAxis("Modifier") > 0)){control.manySelect(h);}else if((Input.GetAxis("Modifier") < 0)){control.deselect(h);}}
        //multiselection is tracked, behaves similarly to image editing software
        if(toolset[4]){if(control.s){if(control.multiSelect.Contains(h)){h.GetComponent<MapTileObject>().applyPaint(paint); d = true;}}
      ...
      if(toolset[1]){mp = Input.mousePosition;
      ...
      if(firstHit){size.x = rt.rect.width * canvtogrid * mirror.x;
      ...
      else{initmp = mp; //we want reflection between actual behavior and what the interface reports
    ...
    if(Input.GetMouseButtonUp(0)){radDisp.SetActive(false);
      ...
      if(toolset[2]){if(rad != 0){control.radialSelect(rad, control.selected, out control.tempSelectList);}else{r = false;}}
      if(toolset[1] || (toolset[2] && r)){
        if(toolset[3]){if(Input.GetAxis("Modifier") == 0){control.clearSelect(); control.finalizeSelect();}
          else if(Input.GetAxis("Modifier") < 0){control.tempSelectList = filterList(control.tempSelectList,control.multiSelect); control.deselect(control.tempSelectList);}
          else{control.finalizeSelect();}}
        if(toolset[4]){if(control.s){control.tempSelectList = filterList(control.tempSelectList,control.multiSelect);}usePaint(control.tempSelectList, paint); control.tempSelectList.Clear(); d = true;}
        if(toolset[5]){if(control.s){control.tempSelectList = filterList(control.tempSelectList,control.multiSelect);}create(control.tempSelectList); control.tempSelectList.Clear(); d = true;}
        if(toolset[7]){if(control.s){control.tempSelectList = filterList(control.tempSelectList,control.multiSelect);}delete(control.tempSelectList); control.tempSelectList.Clear(); d = true;}
      }}
    //Control speed is modified by mod keys, SHIFTs increase speed, CTRLs decrease
    if((Input.GetAxis("Vertical") > 0) && camMode){camtran.Translate(fw * Time.deltaTime * 100 * ((Input.GetAxis("Modifier") > 0) ? 2 : (Input.GetAxis("Modifier") < 0) ? 0.5f : 1), Space.World);}
    if((Input.GetAxis("Horizontal") < 0) && camMode){camtran.Translate(ri * Time.deltaTime * 100 * -1 * ((Input.GetAxis("Modifier") > 0) ? 2 : (Input.GetAxis("Modifier") < 0) ? 0.5f : 1), Space.World);}
    if((Input.GetAxis("Vertical") < 0) && camMode){camtran.Translate(fw * Time.deltaTime * 100 * -1 * ((Input.GetAxis("Modifier") > 0) ? 2 : (Input.GetAxis("Modifier") < 0) ? 0.5f : 1), Space.World);}
    if((Input.GetAxis("Horizontal") > 0) && camMode){camtran.Translate(ri * Time.deltaTime * 100 * ((Input.GetAxis("Modifier") > 0) ? 2 : (Input.GetAxis("Modifier") < 0) ? 0.5f : 1), Space.World);}
    //Rotation is handled by the scroll wheel while holding CTRL
    //Otherwise scroll wheel handles "zoom"
    if(Input.mouseScrollDelta.y != 0){if(Input.GetAxis("Modifier") < 0){if(control.s){if(Input.mouseScrollDelta.y > 0){foreach(GameObject e in control.multiSelect){e.GetComponent<MapTileObject>().rotRight();}}else{foreach(GameObject e in control.multiSelect){e.GetComponent<MapTileObject>().rotLeft();}}}
        else{if(Input.mouseScrollDelta.y > 0){control.selected.GetComponent<MapTileObject>().rotRight();}else{control.selected.GetComponent<MapTileObject>().rotLeft();}}}
      else{cam.GetComponent<Camera>().orthographicSize = Mathf.Min(Mathf.Max(cam.GetComponent<Camera>().orthographicSize - (Input.mouseScrollDelta.y * 10 * ((Input.GetAxis("Modifier") > 0) ? 2 : (Input.GetAxis("Modifier") < 0) ? 0.5f : 1)),10),200);
      canvtogrid = (float)(cam.GetComponent<Camera>().orthographicSize / 400);}}
    //Middle click moves the camera as well
    if(Input.GetMouseButton(2)){camtran.Translate((Input.GetAxis("Mouse X") * -2 * ri * ((Input.GetAxis("Modifier") > 0) ? 2 : (Input.GetAxis("Modifier") < 0) ? 0.5f : 1)) + (Input.GetAxis("Mouse Y") * -2 * fw * ((Input.GetAxis("Modifier") > 0) ? 2 : (Input.GetAxis("Modifier") < 0) ? 0.5f : 1)), Space.World);}
    //Right click rotates the camera
    if(Input.GetMouseButton(1)){camtran.rotation = Quaternion.Euler(camtran.eulerAngles.x + (Input.GetAxis("Mouse Y") * ((Input.GetAxis("Modifier") > 0) ? 2 : (Input.GetAxis("Modifier") < 0) ? 0.5f : 1)),camtran.eulerAngles.y + (Input.GetAxis("Mouse X") * ((Input.GetAxis("Modifier") > 0) ? 2 : (Input.GetAxis("Modifier") < 0) ? 0.5f : 1)),0);
      fw.x = Mathf.Sin(camtran.eulerAngles.y * Mathf.Deg2Rad); fw.z = Mathf.Cos(camtran.eulerAngles.y * Mathf.Deg2Rad); ri.x = Mathf.Sin((camtran.eulerAngles.y + 90) * Mathf.Deg2Rad); ri.z = Mathf.Cos((camtran.eulerAngles.y + 90) * Mathf.Deg2Rad);}
    if(Input.GetMouseButtonUp(1)){camangleadjust = Mathf.Abs(Mathf.Sin(Mathf.Deg2Rad * camtran.eulerAngles.x));}
    if((Input.GetAxis("Command") != 0)){if(Input.GetKeyDown(KeyCode.A)){foreach (GameObject t in control.grid){control.manySelect(t);}
  ...
  public void setProject(prj p){control.dc.p = p;
  ...
  public void setTexName(string name){texname = name;}
  public void newPaint(){tmat = new Material(bmat);
    texname = "unnamed" + palette.Count;
    namebox.text = texname;}
  public void cancelPaint(){previewTex.SetActive(false); tc.Set(1,1,1,1);}
  public void acceptPaint(){previewTex.SetActive(false);
  ...
  public void pickFlag(int i){flag = i;}
  ...
  public void openFile(){string[] bepis = StandaloneFileBrowser.OpenFilePanel("Select Texture...", control.dc.ddirec,"png",false);
  try{if(bepis[0]!=null){filename = bepis[0];
  path.text = filename;
  ...
  public void setfilename(){lprje.SetActive(false);
    string[] bepis = StandaloneFileBrowser.OpenFilePanel("Select Project File...",control.dc.ddirec,"prj",false);
    try{if(bepis[0]!=null){filename = bepis[0]; loadpath.text = bepis[0];}}
    catch(System.IndexOutOfRangeException){}}
  public void setfilename(string s){filename = s;}
  public void openImage(string path){if(path != ""){try{filename = path;
  StartCoroutine(loadTexFromFile());}
  catch(System.IO.FileNotFoundException){dne.SetActive(true);}
  catch(System.ArgumentException){pnv.SetActive(true);}
  catch(Exception){e.SetActive(true);}}}
  ...
  public IEnumerator<Texture2D> loadImage(string path){Texture2D i = new Texture2D(2,2);
  ...
  public IEnumerator saveImage(string path, Texture2D t){FileStream file = new FileStream(path,FileMode.OpenOrCreate, FileAccess.Write, FileShare.Write);
    ...
    file.Close();
    yield break;}
  public void buildButton(paint p){RectTransform psrt = paintScroll.GetComponent<RectTransform>();
  ...
  public IEnumerator loadButton(int i){RectTransform psrt = paintScroll.GetComponent<RectTransform>();
    psrt.offsetMin = new Vector2(psrt.offsetMin.x, psrt.offsetMin.y - 44);
  ...
  public IEnumerator savePaint(paint p){FileStream file = new FileStream(control.dc.direc + paints + control.dc.p.palettelen + ".pnt",FileMode.Create,FileAccess.ReadWrite);
    ...
    yield break;}
  public IEnumerator loadPalette(){for(int i = 0; i < control.dc.p.palettelen; i++){yield return (loadPaint(i));}
    paint = palette[0]; yield break;
  }
  public IEnumerator loadPaint(int index){paint p = new paint();
    ...
    yield break;}
  //breaking up concurrent access, saves headaches
  ...
  public IEnumerator saveMp(){for(int j = 0; j < control.map.xLen; j++){for(int k = 0; k < control.map.grid.Length / control.map.xLen; k++){control.map.grid[j,k] = control.grid[j,k].GetComponent<MapTileObject>().toCell();}}
  ...
  public IEnumerator newMp(){yield return saveDialog();
  ...
  public IEnumerator newPrj(){yield return saveDialog();
    clearMap();
    control.dc.direc = control.dc.ddirec;
    updateMenus();
    newprjmenu.SetActive(true);}
  public void makeNewPrj(){filename = (filename == "" ? "unnamed" : filename);
  ...
  public IEnumerator loadPrj(){yield return saveDialog();
    clearMap();
    control.dc.direc = control.dc.ddirec;
  ...
  filename = "";}
  public IEnumerator loadPrj(string path){FileInfo fi = new FileInfo(path);
  ...
  public IEnumerator savePrj(){FileStream file = new FileStream(control.dc.direc + control.dc.p.name + ".prj",FileMode.Create,FileAccess.ReadWrite,FileShare.ReadWrite);
  ...
  public void ClearPalette(){foreach(GameObject e in paintlist){Destroy(e);}
  paintlist.Clear();
  palette.Clear();
  ...
  public List<GameObject>filterList(List<GameObject> target, List<GameObject> filter){List<GameObject> ret = new List<GameObject>();
  ...
  public void addtoloadmenu(int i){TMP_Dropdown.OptionData od = new TMP_Dropdown.OptionData(control.dc.p.names[i]);
    loadlist.options.Add(od);}
  //animates the paint previews, alter w to change framespeed
  public IEnumerator aniPreview(){WaitForSeconds w = new WaitForSeconds(.33f);
    int f = 0;
    while(true){yield return w; foreach(GameObject e in paintlist){e.GetComponent<PaintBucket>().setFrame(f); f = (f + 1) % 6;}}}
  ...
  public void usePaint(List<GameObject> l, paint paint){foreach (GameObject t in l)
    {t.GetComponent<MapTileObject>().applyPaint(paint);}}
  ...
  public void setCD(){string[] bepis = StandaloneFileBrowser.OpenFolderPanel("Set Project Directory...",control.dc.direc,false);
