...
public class EditorControl : MonoBehaviour
{
  ...
  IEnumerator Start() {
    ...
    if (control.dc.l && control.dc.p.mapcount > 0) {
      loadmapmenu.SetActive(true); control.GetComponentInChildren<MapLoader>(true).gameObject.SetActive(true);
  ...
  void Update() {
    mod = Input.GetAxis("Modifier");
    cmod = ((mod > 0) ? 2 : (mod < 0) ? 0.5f : 1); 
    zoom = Mathf.Clamp(Input.GetAxis("Zoom"),-.02f,.02f) + Input.mouseScrollDelta.y;
    // initial leftclick
    ...
    if (Input.GetMouseButton(0)) {
      if (toolset[6]) {
        ...
        if (toolset[0]) {
          Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
          if (Physics.Raycast(ray, out hit) && mod != 0) {
            GameObject h = hit.collider.gameObject;
            if (toolset[3]) { if((mod > 0)) control.ManySelect(h); 
              else if ((mod < 0)) control.Deselect(h);
            }
          // multiselection is tracked, behaves similarly to image editing software
            if (toolset[4]) {
              if (control.s) { 
                if (control.multiSelect.Contains(h)) {
                  h.GetComponent<MapTileObject>().ApplyPaint(paint); d = true;
                }
              }
              else {
                h.GetComponent<MapTileObject>().ApplyPaint(paint); d = true;
      ...
      // I've tried having this only operate on release, doesn't work, so it lives here instead
        ...
        if (Physics.Raycast(ray, out hit)) {
          ...
          selRect.GetComponent<Transform>().rotation =
            Quaternion.Euler(0,cam.GetComponent<Transform>().rotation.eulerAngles.y,0);
          firstHit = true;
          }
        }
      }
        if (toolset[2]) {
          rad = Mathf.FloorToInt(Mathf.Abs((Vector2.Distance(initmp, Input.mousePosition) * canvtogrid))/10);
    ...
    if (Input.GetMouseButtonUp(0)) {
      ...
      if (toolset[0]) {
        if (toolset[3]) {
          Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
          if (Physics.Raycast(ray, out hit)) {
            if (mod > 0) control.ManySelect(hit.collider.gameObject);
            else if (mod < 0) control.Deselect(hit.collider.gameObject);
      ...
      if (toolset[1] || (toolset[2] && r)) {
        if (toolset[3]) {
          if (mod == 0) {control.ClearSelect(); control.FinalizeSelect();}
          else if (mod < 0) {
    ...
    if ((Input.GetAxis("Vertical") > 0) && camMode) {
      camtran.Translate(fw * Time.deltaTime * 100 * cmod, Space.World);
      }
    if((Input.GetAxis("Horizontal") < 0) && camMode) {
      camtran.Translate(ri * Time.deltaTime * 100 * -1 * cmod, Space.World);
      }
    if((Input.GetAxis("Vertical") < 0) && camMode) {
      camtran.Translate(fw * Time.deltaTime * 100 * -1 * cmod, Space.World);
      }
    if((Input.GetAxis("Horizontal") > 0) && camMode) {
      camtran.Translate(ri * Time.deltaTime * 100 * cmod, Space.World);
      }
    ...
    if (zoom != 0) {
      if (mod < 0) {
        if (control.s) {
          if (zoom > 0) {
            foreach (GameObject e in control.multiSelect) {
              e.GetComponent<MapTileObject>().RotRight();
            }
          }
          else {
            foreach(GameObject e in control.multiSelect) {
              e.GetComponent<MapTileObject>().RotLeft();
            }
          }
        }
        else {
          if (zoom > 0) control.selected.GetComponent<MapTileObject>().RotRight();
    ...
    if (Input.GetMouseButton(2)) {
      camtran.Translate(
        (Input.GetAxis("Mouse X") * -2 * ri * cmod) +
        (Input.GetAxis("Mouse Y") * -2 * fw * cmod), Space.World);
    }
    // Right click rotates the camera
    if (Input.GetMouseButton(1)) {
      camtran.rotation = Quaternion.Euler(
        camtran.eulerAngles.x + (Input.GetAxis("Mouse Y") * cmod), // x
        camtran.eulerAngles.y + (Input.GetAxis("Mouse X") * cmod), // y
        0); // z
    ...
    if ((Input.GetAxis("Command") != 0)) {
      if (Input.GetKeyDown(KeyCode.A)) {
        foreach (GameObject t in control.grid) control.ManySelect(t);
  ...
  public void Create(GameObject e) {
    if (e.GetComponent<MapTileObject>().act) control.Create(e,paint);
    else control.Restore(e,paint);
  }
  public void Create(List<GameObject> l) {
    foreach (GameObject e in l) Create(e);
    d = true;
  }
  public void Delete(GameObject e) {
    if (control.extras.Contains(e)) Destroy(e);
    else control.Remove(e);
  }
  public void Delete(List<GameObject> l) {
    foreach (GameObject e in l) Delete(e);
    d = true;
  }
  ...
  public IEnumerator AniPreview() {
    ...
    while (true) {
      yield return w; 
      foreach(GameObject e in paintlist) {
        e.GetComponent<PaintBucket>().SetFrame(f); 
        f = (f + 1) % 6; 
      }
    }
  }
  ...
  public void SetCD() {
    string[] TSA = StandaloneFileBrowser.OpenFolderPanel("Set Project Directory...",control.dc.direc,false);
  ...
  public void BuildButton(Paint p) {
    ...
    GameObject b = Instantiate<GameObject>(
      ...
      new Vector3(
        psrt.position.x,
        psrt.position.y + 13 - (22 * p.index),
        psrt.position.z),
      Quaternion.identity,psrt);
    paintlist.Add(b);
  ...
  private IEnumerator BuildPreview(PaintBucket pb) {
    previewTile.SetActive(true); previewCam.gameObject.SetActive(true);
  ...
  public IEnumerator LoadButton(int i) {
    ...
    if (check.Exists) {
      OutputCoroutine<Texture2D> c = new OutputCoroutine<Texture2D>(this, LoadImage(control.dc.direc + previews + palette[i].name + ".png"));
      yield return c.c;
      pb.preview = c.r;
      pb.SetFrame(0);
      pb.image.texture = pb.copy;
  ...
  public void OpenTexture() {
    string[] TSA = StandaloneFileBrowser.OpenFilePanel("Select Texture...", control.dc.ddirec,"png",false);
    try { if(TSA[0]!=null) {
      filename = TSA[0];
      path.text = filename;
  ...
  private IEnumerator LoadTexFromFile() {
    pnv.SetActive(false); dne.SetActive(false); e.SetActive(false);
  ...
  public void ClearMap() {
    int zLen = control.grid.Length / control.map.xLen;
    for (int j = 0; j < control.map.xLen; j++) {
      for (int k = 0; k < zLen; k++) {
        GameObject.Destroy(control.grid[j,k]);
      }
    }
  ...
  public IEnumerator UpdateMp() {
    MapGenerator g = control.GetComponentInChildren<MapGenerator>(true);
  ...
  public void LoadAProject() {
    StartCoroutine(LoadPrj(filename));
    filename = "";
  }
  public IEnumerator LoadPrj(string path) {
    FileInfo fi = new FileInfo(path);
    if (fi.Exists) {
      FileStream file = fi.Open(FileMode.Open, FileAccess.ReadWrite);
  ...
  public IEnumerator SavePrj() {
    FileStream file = new FileStream(control.dc.direc + control.dc.p.name + ".prj",
